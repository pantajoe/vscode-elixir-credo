...
import { Task, TaskQueue, type TaskToken } from '../../task-queue'
...
describe('TaskQueue', () => {
  ...
  context('when multiple tasks with different urls are added to queue', () => {
    ...
    context('when selecting next task', () => {
      let taskWrapper3: TestTaskWrapper

      beforeEach(() => {
        taskWrapper3 = new TestTaskWrapper(vscode.Uri.file('/path/to/file3'))
      })
  ...
  context('when multiple tasks with same url are added to queue', () => {
    let anotherTaskWrapper: TestTaskWrapper

    beforeEach(() => {
      anotherTaskWrapper = new TestTaskWrapper(taskWrapper.url)
    })
